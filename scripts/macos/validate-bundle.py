...
assert p['CFBundleIdentifier'] == 'io.github.praneshsivasankaran.llumi', 'Unexpected bundle identifier'
assert p['CFBundleExecutable'] == 'Llumi', 'Unexpected executable'
for key in ('CFBundleShortVersionString', 'CFBundleVersion', 'LlumiReleaseVersion'):
    assert re.fullmatch(r'[0-9][A-Za-z0-9.\-]*', p[key]), 'Invalid version'
assert p['CFBundleShortVersionString'] == p['LlumiReleaseVersion'] == '1.1.1', 'Production version mismatch'
...
arches = set(subprocess.check_output(['/usr/bin/lipo', '-archs', str(app/'Contents/MacOS/Llumi')], text=True).split())
assert arches == {'arm64', 'x86_64'}, f'Unexpected architectures: {arches}'
print('Bundle:', p['LlumiReleaseVersion'], 'build', p['CFBundleVersion'], 'arm64 + x86_64; Intel runtime not physically validated')
